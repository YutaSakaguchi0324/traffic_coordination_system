...
## 概要
自動運転車が車線変更をスムーズに行えるように、自動で車間距離を調整するシステムを作成した。（詳しくはYutaSakaguchi Article.pdfまたはYutaSakaguchi Masterthesis.pdf参照）
...
## 最適化の数理モデル
このシステムには数理最適化が使われており、そのモデルの説明を記す。
...
交通シミュレーションでは、先行車の速度や車間距離から安全が確保できる加速度を計算するモデルが存在する。そのモデルで計算される加速度 $a_{\rm cfm}$ 以下であれば、前方車と衝突しないことが保証される。車追従モデルには様々なモデルがあるが、今回はIDM(intelligent driver model)というモデルを使った。
...
$l$ は車の長さで、 $g_{\rm min}$ は最小車間距離、 $T$ は前方車両の現在位置に自車が到達するまでの時間である。
